...
export async function POST(request: NextRequest) {
  ...
  const checkoutReference = `GHC-TEST-050-${Date.now()}-${randomUUID().slice(0, 8)}`
  ...
  try {
    ...
    const checkoutResponse = await fetch('https://api.sumup.com/v0.1/checkouts', {
      ...
      body: JSON.stringify({
        ...
        description: 'GHC Nutrition · producto de prueba SumUp · 0,50 EUR',
        customer_id: sumupCustomerId,
    ...
    console.error('SumUp 0.50 test checkout error', error)
    if (order) {
      try {
        await settleOrder(order, 'FAILED')
      } catch (settleError) {
    ...
    return NextResponse.json({ error: 'No se pudo iniciar la prueba de 0,50 €.' }, { status: 502 })
  }
}
